...
async function purifier(item) {
    console.log("upload.js: purifier");
...
async function uploader(item, watermark = null, _replay = false) {
    console.log("upload.js: uploader");
 ...
 * @export
 * @param {Blob|File} blob
 * @param {Watermark|null} [watermark] await requestWeiboWatermark(ShareStore.get(K_WEIBO_INHERITED_WATERMARK));
 * @return {Promise<PackedItem>}
 * @reject {Error|{login: boolean, terminable: boolean}}
 */
export async function requestUpload(blob, watermark) {
    console.log("upload.js: requestUpload", blob);
